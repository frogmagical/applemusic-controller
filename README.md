...
3. **起動**
   ...
   - Apple Music は**起動中の出力先変更を反映しない**ため、トグル切り替え直後に再起動の確認ダイアログが出ます。Yes で Apple Music が自動再起動されます。ルーティング設定は永続なので、この再起動が必要なのはトグルを切り替えた時だけです(次回以降の起動では不要)
...
- **Apple Music は起動中の出力先変更を反映しません。** ルーティングトグルの切り替え後は Apple Music の再起動が必要です(本アプリが確認の上で自動再起動します)。設定は永続のため、通常この再起動は切り替え時の1回だけです
