"""tkinter GUI: now-playing display, transport, and key/pitch/tempo sliders."""
...
class App:
    ...
    def _build_sliders(self) -> None:
        ...
        self.var_speed = tk.DoubleVar(value=100)
        ...
        def set_speed(v: float) -> None:
            v = round(v)
            self.var_speed.set(v)
            self.engine.set_speed(v / 100.0)

        def reset_speed() -> None:
            self.var_speed.set(100)
            set_speed(100)
        ...
        self._slider_row(f, "Speed", self.var_speed, 50, 150,
                         lambda v: f"{round(v)} %", set_speed, reset_speed)
